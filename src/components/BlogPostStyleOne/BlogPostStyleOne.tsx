        ...
        <Container>
          <Row>
            ...
            <Col xs={8} className="col-one">
              ...
              {/* Blog Posts */}
              {BlogPostStyleOneContent.map((item, index) => (
                <div className="pb-5" key={index}>
                  <h4>{item.title}</h4>
                  <p>
                    {item.date} by <a href="#">{item.author}</a>
                  </p>
                  <Image
                  ...
                  <p className="pt-3 pb-3">
                    <i>{item.shortContent}</i>
                  </p>
                  <p>{ContentLoremIpsum.paragraphOne}</p>

                  <p>{ContentLoremIpsum.paragraphTwo}</p>
